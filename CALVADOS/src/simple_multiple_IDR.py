...
record = str(sys.argv[1]) # protein name
sequence = str(sys.argv[2])
temp = 298.15  # unit: K
pH = 7.4
ionic =  0.15 #0.07 # unit: M
...
if not isIDP:
    ...
    #for more information, please check step3 in "Run single-chain simulations with CALVADOS 3" from https://github.com/KULL-Centre/_2024_Cao_CALVADOSCOM/blob/main/README.md
    
    domain_boundaries = {record: [[11, 89]] } # this is one domain boundary
                                  # [[230,240],[250,351]]  # a nested list as an example of excluding a small loop (241-249) within domain
                                  #[105, 179]  # this is another domain boundary
    
    #if doing an MDP, fill in path for atomistic structure
    path2pdb = ''
...
if customized_simulation_time == None:  # if None, script will use the default simulation duration depending on the IDR sequence length;
    ...
    nframes = 300  # total number of frames to keep for each replica (exclude discarded frames)
...
########################################################################################################################
dataset = f"Data/{purpose}{CoarseGrained}_{cutoff}_trial{trial}_{record}"
print(dataset)
